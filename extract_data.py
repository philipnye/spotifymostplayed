...
def spotify_track_search(row):
	artists = re.split(r'\sand\s|\s&\s|,\s|\sfeaturing\s', df.at[index, 'artist'])

	collaboration = False
	if re.findall(r'\sand\s|\s&\s|,\s|\sfeaturing\s', df.at[index, 'artist']):		# this works fine for the current top 100, but we'll come unstuck when new artist '666 featuring & 99' hits the streets. 'and' is case sensitive and ignores 'And's in artist names.
		collaboration = True

	q_title = df.at[index, 'title'].replace('\'', '')		# the API is tripped up by apostrophes
	...
	for track_item in result['tracks']['items']:
		spotify_title = re.split(r' \(feat\..*\)', track_item['name'])[0]		# ditch details of collaborators from the track name that Spotify sometimes adds e.g. '(feat. Young Thug)'
		spotify_title = re.split(r' \(with .*\)', spotify_title)[0]		# handle a couple more cases of collaborators being listed in the track name. Fine until there's a track title that legitimately contains this string pattern...
		if spotify_title.lower() == df.at[index, 'title'].lower():		# in the absence of an exact match functionality in the API search
			if track_item['album']['release_date_precision'] == 'day':		# some albums only have precision in years or months, not days
				if datetime.strptime(track_item['album']['release_date'], '%Y-%m-%d') < df.at[index, 'release_date']:
					df.at[index, 'title'] = spotify_title		# go with the styling conventions applied in Spotify
				...
				if int(track_item['album']['release_date']) < df.at[index, 'release_date'].year:
					df.at[index, 'title'] = spotify_title
					df.at[index, 'artists'].clear()
					df.at[index, 'spotify_artist_IDs'].clear()
					for artist in track_item['artists']:
						df.at[index, 'artists'].append(artist['name'])
				...
				print(spotify_title + ' has release date precision of ' + track_item['album']['release_date_precision'])
...
def genius_track_search(row):
	# q_title =
	# q_artists =
	q_str = 'track:"' + q_title + '" artist:' + q_artists		# spotipy accepts artist names separated by a space
	result = genius.search_song('Andy Shauf', max_songs = 3, sort = 'title')


df = pd.read_csv('list_short.csv', index_col = 0, encoding = 'cp1252')		# standard Windows encoding
